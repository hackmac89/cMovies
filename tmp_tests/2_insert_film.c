...
int main(int argc, char** argv)
{
	...
	if( !(addBasicInfoToMovieContext(&test, "Star Wars: Das Erwachen der Macht", "Science-Fiction", "02:15:00", 
			"Mehr als drei Jahrzehnte nach „Star Wars 6 – Die Rückkehr der Jedi-Ritter“ wurde das Imperium durch die „Erste Ordnung“ abgelöst, eine ebenfalls diktatorische Organisation mit anderem Namen, die Krieg gegen den Widerstand führt...",
			"BR-RIP", "HDD", 2015, 0, 8, false, false)) )
		printError("[!] FEHLER beim eintragen der Basisinformationen. Breche ab...", true);
	/* Add director(s) and actor(s) to the movie context */
	addDirectorToMovieContext(&test, "J. J. Abrams");
	addActorToMovieContext(&test, "Daisy Ridley");
	addActorToMovieContext(&test, "John Boyega");
	addActorToMovieContext(&test, "Oscar Isaac");
	addActorToMovieContext(&test, "Adam Driver");
	addActorToMovieContext(&test, "Harrison Ford");
	addActorToMovieContext(&test, "Carrie Fisher");
	addActorToMovieContext(&test, "Mark Hamill");
	addActorToMovieContext(&test, "Lupita Nyong’o");
	addActorToMovieContext(&test, "Andy Serkis");
	addActorToMovieContext(&test, "Domhnall Gleeson");
